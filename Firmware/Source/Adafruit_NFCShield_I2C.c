/**************************************************************************/
/*! 
    @file     Adafruit_NFCShield_I2C.c
    @author   Adafruit Industries, Modified by Tijn Kooijmans
...
#define PORT_RST    cbPIO_PORT_0
#define PORT_I2C    cbPIO_PORT_1
...
void NFCShield_begin() {

  pinMode(PIN_IRQ, PORT_IRQ, cbPIO_INPUT_FLOATING);
  pinMode(PIN_RST, PORT_RST, OUTPUT);
  
  // we use a software I2C, which doesn't consume power when not used
  SWI2C_init();
...
void NFCShield_stop() {
  ...
  pinMode(PIN_SDA, PORT_I2C, OUTPUT);
  pinMode(PIN_SCL, PORT_I2C, OUTPUT);
  digitalWrite(PIN_SDA, PORT_I2C,LOW);
  digitalWrite(PIN_SCL, PORT_I2C,LOW);
...
uint32_t getFirmwareVersion(void) {
  ...
  if (! sendCommandCheckAck(pn532_packetbuffer, 1, DEFAULT_TIMEOUT))
    return 0;
...
boolean writeGPIO(uint8_t pinstate) {
  ...
  if (! sendCommandCheckAck(pn532_packetbuffer, 3, DEFAULT_TIMEOUT))
    return 0x0;
...
boolean SAMConfig(void) {
  ...
  if (! sendCommandCheckAck(pn532_packetbuffer, 4, DEFAULT_TIMEOUT))
     return false;
...
boolean setPassiveActivationRetries(uint8_t maxRetries) {
  ...
  if (! sendCommandCheckAck(pn532_packetbuffer, 5, DEFAULT_TIMEOUT))
    return 0x0;  // no ACK
...
boolean readPassiveTargetID(uint8_t cardbaudrate) {
...
*/
/**************************************************************************/
boolean pollPassiveTargetID(uint8_t * uid, uint8_t * uidLength) {
...
uint8_t mifareclassic_AuthenticateBlock (uint8_t * uid, uint8_t uidLen, uint32_t blockNumber, uint8_t keyNumber, uint8_t * keyData)
{
  ...
  osal_memcpy (_key, keyData, 6); 
  osal_memcpy (_uid, uid, uidLen); 
  _uidLen = uidLen;  
  ...
  osal_memcpy (pn532_packetbuffer+4, _key, 6);
  for (i = 0; i < _uidLen; i++)
  {
...
uint8_t mifareclassic_ReadDataBlock (uint8_t blockNumber, uint8_t * data)
{
  ...
  osal_memcpy (data, pn532_packetbuffer+8, 16);
...
uint8_t mifareclassic_WriteDataBlock (uint8_t blockNumber, uint8_t * data)
{
  ...
  pn532_packetbuffer[3] = blockNumber;            /* Block Number (0..63 for 1K, 0..255 for 4K) */
  osal_memcpy (pn532_packetbuffer+4, data, 16);          /* Data Payload */

  /* Send the command */
  if (! sendCommandCheckAck(pn532_packetbuffer, 20, DEFAULT_TIMEOUT))
  {
    return 0;
  }  
  delay(10);
...
uint8_t mifareultralight_ReadPage (uint8_t page, uint8_t * buffer)
{
  ...
  if (pn532_packetbuffer[7] == 0x00)
  {
    ...
    /* bytes                                              */
    osal_memcpy (buffer, pn532_packetbuffer+8, 4);
  }
  else
  {
    return 0;
...
boolean readackframe(void) {
  ...
  return (TRUE == osal_memcmp((char *)ackbuff, (char *)pn532ack, 6));
}
...
/**************************************************************************/
uint8_t wirereadstatus(void) {
  uint8_t x = digitalRead(PIN_IRQ, PORT_IRQ);
...
void wiresendcommand(uint8_t* cmd, uint8_t cmdlen) {
  ...
  uint8_t *sendData  = osal_mem_alloc((8 + cmdlen) * sizeof(uint8_t));
  ...
  sendData[i++] = (PN532_PREAMBLE);
  sendData[i++] = (PN532_PREAMBLE);
  sendData[i++] = (PN532_STARTCODE2);

  sendData[i++] = (cmdlen);
  sendData[i++] = (~cmdlen + 1);
 
  sendData[i++] = (PN532_HOSTTOPN532);
  checksum += PN532_HOSTTOPN532;

  for (uint8_t j=0; j<cmdlen-1; j++) {
   sendData[i++] = (cmd[j]);
   checksum += cmd[j];
  }
  
  sendData[i++] = (~checksum);
  sendData[i++] = (PN532_POSTAMBLE);
  
  SWI2C_writeBlock(PN532_I2C_ADDRESS, i, sendData);

  osal_mem_free(sendData);
} 
...
boolean inDataExchange(uint8_t * send, uint8_t sendLength, uint8_t * response, uint8_t * responseLength) {
  ...
  if (!sendCommandCheckAck(pn532_packetbuffer,sendLength+2, DEFAULT_TIMEOUT)) {
    return false;
  }
